...
def show(self,page):
    ...
    resp = self.get_resp(
            '/login/',
        data=dict(username='shakeelshakir123', password='KINGrdf1122#'))
    ...
    # s.post('http://localhost:8088/login/',data=dict(username='shakeelshakir123', password='KINGrdf1122#',csrf_token=csrf_token))
    # g=s.get('http://localhost:8088/superset/explore_json/table/3/?form_data={"datasource":"3__table","viz_type":"big_number_total","slice_id":66,"granularity_sqla":"ds","time_grain_sqla":"Time+Column","since":"100+years+ago","until":"now","metric":"sum__num","subheader":"total+female+participants","y_axis_format":".3s","where":"","having":"","filters":[{"col":"gender","val":["girl"],"op":"in"}]}')
    return resp
